...
drop table if exists purchasEvents;
create table purchasEvents
using parquet
with purchaseSourse
as
(
  select
  ecommerce,
  to_date(timestamp(event_previous_timestamp/1e6)) as previousEventDate,
  to_date(timestamp(event_timestamp/1e6)) as eventDate,
  geo,
  user_id as userId
  from
  eventsRaw
...
select
double(ecommerce.purchase_revenue_in_usd) as purchases,
previousEventDate,
eventDate,
geo.city as city,
geo.state as state,
...
select count(*) from purchasEvents
...
--TODO
select purchases, city, state from purchasEvents
...
select
  date_format(eventDate, 'D') day,
  mean(purchases) totalPurchase
from
  purchasEvents
...
describe extended usersRaw

-- COMMAND ----------

--TODO

CREATE OR REPLACE TEMPORARY VIEW confirmedEmails AS 
  SELECT 
    DISTINCT user_id as userId, 
    to_date(TIMESTAMP(user_first_touch_timestamp/1e6)) as userFirstTouchDate,
    email
  FROM usersRaw
  where email is not NULL
  order by userFirstTouchDate desc;
  
select count(*) from confirmedEmails
...
SELECT *
FROM purchasEvents 
JOIN confirmedEmails ON purchasEvents.userId = confirmedEmails.userId
